# =========================================================
# Strategy Version: v1.7.0 双引擎 (Dragon + 首板低开)
# Updated: 2026-07-06
# Source From: 战车A龙头3_v1.6.0.py (继承 V1.5.0/V1.6.0 全部5处改动)
#
# === V1.7.0 新增: 策略B「首板低开」(治"空仓期资金闲置") ===
# 来源: 聚宽 post/54934(种咖啡果得咖啡因), wywy1995 首板低开一脉, 熊市表现稳
# 定位: A(Dragon)=主升期武器(2023胜率6%/2026胜率49%,只在情绪牛有edge);
#       B(首板低开)=全天候引擎,填补A空仓83%的资金闲置,熊市正收益
# 资金分配: A缩到55%上限 / B占40% / 5%缓冲(b_capital_ratio, TUNABLE)
# 隔离设计: B持仓注册 positions_meta(strategy='B'),A侧所有函数(槽位/分钟止损/
#   尾盘加仓/孤儿清扫/sell_only)均按 meta strategy=='A' 过滤,B自动隔离;
#   gap_down_stop_loss 补了 skip B。B不受 A 的空仓闸/月刹车约束(全天候定位)。
# B规则(照搬原策略): 昨日首板(非连板)+60日相对位置<=0.5+换手<20%+左侧压力位过滤
#   +竞价量比<0.10 → 今开盘低开-4%~-3% → 小市值优先买入(最多5只)
#   → 次日11:28盈利即止盈 / 14:50无条件清(涨停继续持有)
# 状态: UNVERIFIED — 必须聚宽回测(2023-2026长区间)验证组合效果后 Wallace 拍板
# =========================================================
# (以下为 v1.6.0 原头部)
# Strategy Version: v1.6.0
# Updated: 2026-07-06
# Status: research/backtest — UNVERIFIED, 必须在聚宽回测(含2023-2026长区间)验证后才可考虑实盘
# Main File: 战车A龙头3_v1.6.0.py
# Source From: 战车A龙头3_v1.5.0.py (继承 V1.5.0 全部3处改动)
#
# === V1.6.0 新增改动(治"弱年开仓日亏钱",基于 v1.4.0D 2023-2026 长回测诊断) ===
# 诊断结论(log/jq_v140D_20230101_20260622.log.txt):
#   - 83%天未开仓不是病(门槛挡掉的多是亏钱日);病是"开了仓的日子在亏"(2023 −39.6%)
#   - 0.70固定分数线按2026高分行情校准,2023/24分数分布整体偏低,勉强过线的
#     "冷市里的相对热日"正是亏钱主力
# [改动4] regime分层分数线 —— 弱市要求更强的信号才出手
#   dragon_min_score: 固定0.70 → bull 0.70 / neutral 0.72 / bear 0.78 (全TUNABLE)
#   方向: 只收紧不放松(bull维持原值),2026行为几乎不变,2023/24/25被更强过滤
# [改动5] 月度亏损刹车 —— 给单月亏损封顶,截断坏年份的持续放血
#   当月组合净值回撤 <= -monthly_loss_brake_pct(默认6%) → 当月停止新开仓(持仓正常管理)
#   下月自动复位。-39.6%的2023 = 多个亏损月连击,月刹车把每月亏损封在-6%~-8%附近
#
# 诚实标注: 两个改动都是"只减少出手,不增加出手";阈值(0.72/0.78/6%)是先验起点
#   非拟合最优,必须在 2023-2026 长区间验证(重点看2023能否从-39.6%大幅收窄、
#   2026收益保住大头),由 Wallace 回测校准后拍板。
# =========================================================
#
...
def make_global_config():
    ...
    return {
        ...
        # --- [V1.6.0 改动5] 月度亏损刹车 ---
        # 当月组合净值回撤 <= -monthly_loss_brake_pct → 当月停止新开仓(持仓正常管理),
        # 下月自动复位。目的: 给单月亏损封顶,截断2023式坏年份的持续放血。TUNABLE。
        'monthly_loss_brake_enable': True,
        'monthly_loss_brake_pct': 0.06,

        # --- [V1.7.0] 策略B: 首板低开(全天候引擎) ---
        'b_enable': True,
        'b_capital_ratio': 0.40,     # B占总资产比例(A上限相应缩到0.55)
        'b_num_stock': 5,            # B最大持股数
        'b_open_range': [0.96, 0.97],  # 低开窗口: -4% ~ -3%
        'b_low_rp': 0.5,             # 60日相对位置上限
        'b_rp_watch_days': 60,
        'b_filter_turnover': True,   # 换手率过滤
        'b_turnover_days': 1,
        'b_turnover_limit': 0.20,
        'b_filter_open_vol': True,   # 竞价量比过滤
        'b_open_vol_limit': 0.10,
        'b_filter_left_res': True,   # 左侧压力位过滤
        'b_left_res_days': 100,
        'b_left_res_level': 5,
...
def make_strategy_A_config():
    ...
    return {
        ...
        # [V1.7.0] A引擎仓位整体缩到55%上限,给B(首板低开)让出40%。TUNABLE。
        'top1_pos_ratio': 0.28,
        'top2_pos_ratio': 0.20,
        'max_single_stock_ratio': 0.40,
        'max_total_position_ratio': 0.55,
        'max_initial_position_ratio': 0.45,
        ...
        # [V1.6.0 改动4] regime分层分数线: 弱市要求更强信号才出手(只收紧不放松)。
        # 诊断: 0.70按2026高分行情校准;2023/24分数整体偏低,勉强过0.70的
        # "冷市相对热日"是亏钱主力。bear提到0.78、neutral提到0.72。全TUNABLE。
        'dragon_min_score_bull': 0.70,
        'dragon_min_score_neutral': 0.72,
        'dragon_min_score_bear': 0.78,
...
def schedule_all(context):
    ...
    # [V1.7.0] 策略B: 首板低开(选股09:15/竞价后买09:26/次日11:28止盈/14:50清仓)
    run_daily(b_prepare, '09:15')
    run_daily(b_buy, '09:26')
    run_daily(b_stop_profit, '11:28')
    run_daily(b_stop_loss, '14:50')
...
def _evaluate_monthly_loss_brake(context):
    """[V1.6.0 改动5] 月度亏损刹车。返回 (braked, month_ret)。
    当月组合净值(总资产)相对月初回撤 <= -monthly_loss_brake_pct → 当月停新开仓。
    月初基准在跨月后第一次调用时自动复位(惰性更新,无需额外调度)。
    """
    cfg = g.cfg
    if not cfg.get('monthly_loss_brake_enable', True):
        return False, 0.0
    try:
        tv = float(context.portfolio.total_value)
    except Exception:
        return False, 0.0
    cur_month = (context.current_dt.year, context.current_dt.month)
    if getattr(g, 'mlb_month', None) != cur_month:
        g.mlb_month = cur_month
        g.mlb_month_start_value = tv
    start = float(getattr(g, 'mlb_month_start_value', 0.0) or 0.0)
    if start <= 0:
        g.mlb_month_start_value = tv
        return False, 0.0
    month_ret = tv / start - 1
    if month_ret <= -cfg.get('monthly_loss_brake_pct', 0.06):
        return True, month_ret
    return False, month_ret
...
def get_A_mode(context):
    ...
    # [V1.6.0 改动5] 月度亏损刹车: 当月净值回撤超限则当月停新开仓
    braked, month_ret = _evaluate_monthly_loss_brake(context)
    if braked:
        g.diag['monthly_loss_brake'] = 1
        log.info(
            "MONTHLY_LOSS_BRAKE|active=1|month_ret={:.2f}%|brake_pct=-{:.1f}%|"
            "action=no_new_open_this_month".format(
                month_ret * 100,
                g.cfg.get('monthly_loss_brake_pct', 0.06) * 100
            )
        )
        return 'idle', {'max_hold': 0}
    g.diag['monthly_loss_brake'] = 0
    ...
    for stock in stocks:
        try:
            ...
            if not meta or meta.get('pending_exit', False) or meta.get('last_exit_date') == today:
                continue
            # [V1.7.0] B持仓(首板低开)不走A的缺口止损,由B自己的11:28/14:50出场管理
            if meta.get('strategy') == 'B':
                continue
...
def score_candidates_A(context, candidates, mode='dragon', pos_ratio=None):
    ...
    eligible = []
    cfg_a = g.strategies['A']['config']
    # [V1.6.0 改动4] regime分层分数线: bull 0.70 / neutral 0.72 / bear 0.78
    _regime = getattr(g, 'market_regime', 'neutral')
    min_score = cfg_a.get(
        'dragon_min_score_%s' % _regime,
        cfg_a.get('dragon_min_score', 0.70))
...
# =========================================================
# [V1.7.0] 策略B: 首板低开 (来源: 聚宽post/54934, wywy1995一脉)
# 规则: 昨日首板(非连板) + 60日相对位置<=0.5 + 换手<20% + 左侧压力位过滤
#       + 竞价量比<0.10 → 低开-4%~-3%买入(小市值优先, 最多5只)
#       → 次日11:28盈利止盈 / 14:50无条件清仓(涨停继续持有)
# 隔离: 持仓注册 positions_meta(strategy='B'); 不受A的空仓闸/月刹车约束(全天候)
# =========================================================

def _b_holdings(context):
    return set(s for s in context.portfolio.positions
               if g.positions_meta.get(s, {}).get('strategy') == 'B')


def b_prepare(context):
    """09:15 选股: 昨日首板 -> 非连板 -> 低位 -> 换手 -> 左侧压力位"""
    g.b_stock_list = []
    if not g.cfg.get('b_enable', True):
        return
    try:
        date = str(context.previous_date)
        d_prev = context.previous_date
        sec = get_all_securities('stock', date)
        codes = [s for s in sec.index
                 if s[0] not in ('4', '8') and s[:2] != '68'
                 and (d_prev - sec.loc[s, 'start_date']).days > 250]
        cd = get_current_data()
        initial = [s for s in codes
                   if not cd[s].is_st and 'ST' not in cd[s].name
                   and '*' not in cd[s].name and '退' not in cd[s].name
                   and not cd[s].paused]
        # 昨日涨停
        df = get_price(initial, end_date=date, frequency='daily',
        ...
        if not hl_list:
            return
        # 剔除连板(近10日尾部连续涨停>=2)
        df10 = get_price(hl_list, end_date=date, frequency='daily',
                         fields=['close', 'high_limit'], count=10,
                         panel=False, fill_paused=False, skip_paused=False)
        hlmark = (df10['close'] == df10['high_limit'])
        lb = set()
        for s in hl_list:
            m = hlmark[df10.code == s].tolist()
            last0 = -1
            ...
            if sum(m[last0 + 1:]) >= 2:
                lb.add(s)
        stock_list = [s for s in hl_list if s not in lb]
        if not stock_list:
            return
        # 相对位置
        dfp = get_price(stock_list, end_date=date,
                        fields=['high', 'low', 'close'],
                        count=g.cfg.get('b_rp_watch_days', 60),
                        fill_paused=False, skip_paused=False, panel=False).dropna()
        close = dfp.groupby('code')['close'].last()
        high = dfp.groupby('code')['high'].max()
        low = dfp.groupby('code')['low'].min()
        rp = (close - low) / (high - low)
        stock_list = list(rp[rp <= g.cfg.get('b_low_rp', 0.5)].index)
        # 换手率过滤
        if stock_list and g.cfg.get('b_filter_turnover', True):
            data = get_valuation(stock_list, end_date=date,
                                 count=g.cfg.get('b_turnover_days', 1),
                                 fields=['turnover_ratio'])
            tr = data['turnover_ratio'].groupby(data.code).mean()
            stock_list = list(tr[tr < g.cfg.get('b_turnover_limit', 0.20) * 100].index)
        # 左侧压力位过滤
        if stock_list and g.cfg.get('b_filter_left_res', True):
            final = []
            for s in stock_list:
                try:
                    data = attribute_history(s, g.cfg.get('b_left_res_days', 100), '1d',
                                             fields=['high', 'volume'], skip_paused=True)
                    prev_high = data['high'].iloc[-1]
                    idx = next((i - 1 for i, h in enumerate(data['high'][-3::-1], 2)
                                if h >= prev_high), 100)
                    idx += 5
                    vol = data['volume'][-idx:]
                    if len(vol) >= g.cfg.get('b_left_res_level', 5) \
                            and vol.iloc[-1] >= vol.iloc[:-1].max() * 0.8:
                        final.append(s)
                except Exception:
                    continue
            stock_list = final
        g.b_stock_list = stock_list
        log.info("B_PREPARE|count={}|sample={}".format(
            len(stock_list), stock_list[:8]))
    except Exception as e:
        log.error("B_PREPARE_ERROR|{}".format(e))


def b_buy(context):
    """09:26 竞价后: 量比过滤 -> 低开窗口 -> 小市值优先买入"""
    if not g.cfg.get('b_enable', True):
        return
    try:
        stock_list = list(getattr(g, 'b_stock_list', []) or [])
        if not stock_list:
            return
        date = str(context.previous_date)
        cd = get_current_data()
        # 竞价量比过滤
        if g.cfg.get('b_filter_open_vol', True):
            try:
                dnow = context.current_dt.strftime('%Y-%m-%d')
                prev = get_price(stock_list, end_date=context.previous_date,
                                 count=1, frequency='1d', fields=['volume'],
                                 skip_paused=True, panel=False)
                prev = prev.rename(columns={'volume': 'prev_volume'})
                auc = get_call_auction(stock_list,
                                       start_date=dnow + ' 09:15:00',
                                       end_date=dnow + ' 09:26:00',
                                       fields=['time', 'volume', 'current'])
                auc = auc.rename(columns={'volume': 'auction_volume'})
                mg = pd.merge(prev, auc, on='code', how='inner')
                ratio = mg['auction_volume'] / mg['prev_volume']
                stock_list = list(mg[ratio < g.cfg.get('b_open_vol_limit', 0.10)].code)
            except Exception as e:
                log.warning("B_OPEN_VOL_FILTER_ERROR|{}".format(e))
        if not stock_list:
            return
        # 低开窗口
        rng = g.cfg.get('b_open_range', [0.96, 0.97])
        dfc = get_price(stock_list, end_date=date, frequency='daily',
                        fields=['close'], count=1, panel=False,
                        fill_paused=False, skip_paused=True).set_index('code')
        picks = []
        for s in list(dfc.index):
            try:
                op = cd[s].day_open / dfc.loc[s, 'close']
                if rng[0] <= op <= rng[-1]:
                    picks.append(s)
            except Exception:
                continue
        if not picks:
            return
        # 小市值优先
        val = get_valuation(picks, end_date=date, count=1, fields=['market_cap'])
        val = val.sort_values(by='market_cap', ascending=True)
        picks = list(val.code)
        # 买入: B独立预算/槽位, 不碰A持仓
        num = g.cfg.get('b_num_stock', 5)
        b_hold = _b_holdings(context)
        slots = num - len(b_hold)
        if slots <= 0:
            return
        budget_each = context.portfolio.total_value * g.cfg.get('b_capital_ratio', 0.40) / num
        cash = context.portfolio.available_cash
        a_hold = set(s for s in context.portfolio.positions if s not in b_hold)
        for s in [x for x in picks if x not in b_hold and x not in a_hold][:slots]:
            value = min(budget_each, max(0.0, cash))
            if value < 5000:
                break
            o = order_target_value(s, value)
            # 9:26下单9:30才成交,filled此时为0——只要订单被接受就注册meta,
            # 否则B持仓成为无名孤儿被11:26孤儿清扫误杀(v170首测20次ORPHAN_EXIT教训)
            if o is not None:
                g.positions_meta[s] = {
                    'strategy': 'B', 'entry_type': 'b_lowopen', 'stage': 'full',
                    'pending_exit': False, 'last_exit_date': None,
                    'last_exit_reason': '',
                }
                cash -= value
                log.info("B_BUY|stock={}|name={}|value={:.0f}".format(
                    s, get_stock_name_cached(s), value))
    except Exception as e:
        log.error("B_BUY_ERROR|{}".format(e))


def _b_exit(context, only_profit):
    if not g.cfg.get('b_enable', True):
        return
    cd = get_current_data()
    for s in list(_b_holdings(context)):
        try:
            pos = context.portfolio.positions[s]
            if pos.closeable_amount <= 0:
                continue
            last = cd[s].last_price
            if last is None or last <= 0:
                continue
            hl = cd[s].high_limit
            if hl and last >= hl:
                continue  # 涨停不卖, 继续持有
            if only_profit and last <= pos.avg_cost:
                continue
            ret = last / pos.avg_cost - 1 if pos.avg_cost > 0 else 0.0
            o = order_target_value(s, 0)
            if o is not None:
                g.positions_meta.pop(s, None)
                log.info("B_EXIT|stock={}|name={}|reason={}|TRADE_CLOSE_B|ret={:.2f}%".format(
                    s, get_stock_name_cached(s),
                    'b_stop_profit' if only_profit else 'b_stop_loss',
                    ret * 100))
        except Exception as e:
            log.error("B_EXIT_ERROR|stock={}|{}".format(s, e))


def b_stop_profit(context):
    """11:28 盈利止盈(涨停不卖)"""
    _b_exit(context, True)


def b_stop_loss(context):
    """14:50 无条件清仓(涨停不卖)"""
    _b_exit(context, False)
