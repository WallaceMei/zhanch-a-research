...
# - Top1 初始仓位 35%，Top2 初始仓位 25%
# - 初始仓位上限 60%，总仓位上限 75%，单股上限 50%
...
def _evaluate_dragon_intraday_stop(stock, meta, pnl, curr_price, ma5, current_dt, source):
    ...
    if pnl <= 0 or failfast_reason:
        log.info(
            "DRAGON_FAILFAST_CHECK|stock={}|name={}|pnl={:.2f}%|below_ma5_count={}|"
            "reason={}|source={}".format(
                stock, get_stock_name_cached(stock), pnl * 100, meta['below_ma5_count'],
                failfast_reason or 'hold', source
            )
        )
    if failfast_reason:
                ...
                if entry_type == 'dragon_follow':
                    ...
                    if exit_reason:
                        log.info(
                            ...
                            "curr_price={:.3f}|ma5={}|warning_stop={}|delayed_stop={}|"
                            "confirm_stop={}".format(
    ...
    if action == 'sell_all':
        if submit_exit_order(stock, context, reason=reason, ret_snapshot=pnl):
            ...
            if meta.get('entry_type') == 'dragon_follow':
                ...
                log.info(
                    "DRAGON_SELL|stock={}|name={}|reason={}|pnl={:.2f}%|stage={}|"
                    "curr_price={}|ma5={}|warning_stop={}|delayed_stop={}|"
                    "confirm_stop={}".format(
        ...
        if closeable <= 100 or remaining_after_half < 100:
            ...
            if submit_exit_order(stock, context, reason=reason, ret_snapshot=pnl):
                ...
                if meta.get('entry_type') == 'dragon_follow':
                    ...
                    log.info(
                        "DRAGON_SELL|stock={}|name={}|reason={}|pnl={:.2f}%|stage={}|"
                        "curr_price={}|ma5={}|warning_stop={}|delayed_stop=0|"
...
def execute_buy_orders(context, scored, strategy_name, budget, slots_left):
    ...
    for item in scored[:2]:
        ...
        buy_value = total_value * pos_ratio
        ...
        if order_value(stock, buy_value) is not None:
            ...
            log.info(
                ...
                "remaining_initial={:.2f}%|actual_pos_ratio={:.2f}%|market_trend={}|"
...
def sync_position_meta_with_real_positions(context):
    ...
    for stock in list(context.portfolio.positions.keys()):
        ...
        if (meta.get('pending_stage_change')
            ...
            total_filled = max(float(pending_amount_before) - curr_amount, 0)
            previous_filled = float(
                meta.get('pending_partial_filled_amount', 0) or 0
            )
            newly_filled = max(total_filled - previous_filled, 0)
            partial_ret = meta.get('pending_partial_ret_snapshot')
            if (meta.get('entry_type') == 'dragon_follow'
                    and partial_ret is not None and newly_filled > 0):
                meta['dragon_realized_return_weighted'] = (
                    meta.get('dragon_realized_return_weighted', 0.0)
                    + float(partial_ret) * newly_filled
                )
                meta['dragon_realized_amount'] = (
                    meta.get('dragon_realized_amount', 0) + newly_filled
                )
            if newly_filled > 0:
                ...
                if sell_price is not None:
                    sell_value = float(sell_price) * newly_filled
                    cost_value = float(cost_price or 0) * newly_filled
                    pnl_value = sell_value - cost_value
                    meta['total_sell_value'] = (
                        float(meta.get('total_sell_value', 0.0) or 0.0) + sell_value
                    )
                    meta['realized_pnl_value'] = (
                        float(meta.get('realized_pnl_value', 0.0) or 0.0) + pnl_value
                    )
                    meta.setdefault('partial_sell_records', []).append({
                        ...
                        'pnl_value': pnl_value,
                        'reason': meta.get('pending_stage_change_reason', ''),
                    })
                diag_add('partial_sell_confirmed')
                log.info(
                    "PARTIAL_SELL_CONFIRMED|stock={}|name={}|amount={:.0f}|"
                    "sell_value={:.2f}|pnl_value={:.2f}|reason={}".format(
                        stock, get_stock_name_cached(stock), newly_filled,
                        sell_value, pnl_value,
                        meta.get('pending_stage_change_reason', '')
                    )
                )
            meta['pending_partial_filled_amount'] = total_filled

            requested = float(
                meta.get('pending_partial_requested_amount', 0) or 0
            )
            request_filled = requested <= 0 or total_filled + 1e-8 >= requested
            timed_out = (
...
def _log_livermore_add_skip(stock, reason, pnl=None):
    pnl_text = 'NA' if pnl is None else '{:.2f}%'.format(pnl * 100)
    log.info(
        "LIVERMORE_ADD_SKIP|stock={}|name={}|current_pnl={}|reason={}".format(
            stock, get_stock_name_cached(stock), pnl_text, reason
        )
    )
...
def check_livermore_tail_add(context):
    ...
    try:
        ...
        for stock in stocks:
            ...
            after_total_ratio = total_position_ratio + add_ratio
            if after_position_ratio > single_cap + 1e-8:
                _log_livermore_add_skip(stock, 'single_stock_cap', pnl)
                continue
            if after_total_ratio > total_cap + 1e-8:
                _log_livermore_add_skip(stock, 'total_position_cap', pnl)
                continue

            add_value = total_value * add_ratio
            if add_value > context.portfolio.available_cash \
                    or add_value / curr_price < 100:
                _log_livermore_add_skip(stock, 'total_position_cap', pnl)
                continue
            ...
            log.info(
                ...
                "total_position_ratio={:.2f}%|reason=livermore_tail_add".format(
                    ...
                    add_ratio * 100, after_position_ratio * 100,
...
def _get_dragon_stock_list_A_impl(context):
    ...
    # 批量计算近10日日均振幅，避免候选循环内逐票 attribute_history。
    for stock in seed:
        if stock in per_stock:
            per_stock[stock]['avg_range'] = 0.0
    try:
        range_df = get_price(
            list(seed),
            end_date=prev_date,
        ...
        if range_df is not None and not range_df.empty and 'code' in range_df.columns:
            for stock, stock_range_df in range_df.groupby('code'):
                if stock not in per_stock:
                    continue
                valid_range_df = stock_range_df[['high', 'low', 'close']].dropna()
                valid_range_df = valid_range_df[valid_range_df['close'] > 0]
                if len(valid_range_df) >= 5:
                    avg_range = (
                        (valid_range_df['high'] - valid_range_df['low'])
                        / valid_range_df['close']
                    ).mean()
                    if pd.notna(avg_range):
                        per_stock[stock]['avg_range'] = float(avg_range)
    except Exception as e:
        log.warning(
...
def update_dragon_mode(context):
    ...
    top_n = int(g.cfg.get('bigmeat_pool_log_top_n', 5) or 5)
...
def finalize_exited_position(stock, meta):
    ...
    final_sell_value = float(exit_price or 0) * exit_amount
    exit_cost = float(meta.get('pending_exit_cost_price', 0) or 0)
    final_pnl_value = final_sell_value - exit_cost * exit_amount
    total_sell_value = (
        float(meta.get('total_sell_value', 0.0) or 0.0) + final_sell_value
    )
    ...
    log.info(
        ...
        "total_buy_value={:.2f}|total_sell_value={:.2f}|"
        "realized_pnl_value={:.2f}|win={}".format(
            stock, get_stock_name_cached(stock), entry_type,
            final_trade_ret * 100, total_buy_value,
            total_sell_value, realized_pnl_value, int(final_trade_ret > 0)
        )
    )
    ...
    if entry_type == 'dragon_follow' and strategy == 'A':
        ...
        log.info(
            ...
            "dragon_pause_days_left={}|pause_triggered={}|pause_released=0|"
            "ret={:.2f}%".format(
                stock, get_stock_name_cached(stock),
                g.dragon_consecutive_losses,
                g.dragon_circuit_breaker_pause_left,
                int(pause_triggered), final_trade_ret * 100
            )
        )
...
def submit_partial_stage_change(
    ...
    meta['pending_partial_requested_amount'] = amount
    meta['pending_partial_position_amount_before'] = (
        getattr(pos, 'total_amount', 0) if pos is not None else 0
    )
    meta['pending_partial_filled_amount'] = 0
    cost_price = float(getattr(pos, 'avg_cost', 0) or 0) if pos is not None else 0.0
    meta['pending_partial_cost_price'] = cost_price
    meta['pending_partial_sell_price'] = (
