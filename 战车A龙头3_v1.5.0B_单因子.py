# =========================================================
# Strategy Version: v1.5.0B-单因子 (single-factor prefilter)
# Updated: 2026-07-06
# Status: research/backtest — UNVERIFIED 消融实验版
# Main File: 战车A龙头3_v1.5.0B_单因子.py
# Source From: 战车A龙头3_v1.5.0.py
#
# === V1.5.0B 与 V1.5.0 的唯一差异(单变量消融) ===
# 250预筛排序: 6因子加权 → 只按 avg_money_5(5日均成交额)单因子排序取前250。
# 理由: "钱多的优先"是先验常识不是拟合结论——成交额/流动性是所有打板逻辑的根基,
#   几乎不可能过拟合;原6因子里它本来就是最大权重(0.25)。
# 代价: 丢掉 距新高/振幅/涨幅 等形态信息,选出的票偏大偏稳。
# 硬门槛漏斗保持不变(预筛前的 pass/fail 条件一律不动),只换排序依据。
...
def _prefilter_dragon_auction_candidates(
    ...
    # [V1.5.0B] 消融实验: 单因子预筛——只按 avg_money_5(5日均成交额)排序。
    # "钱多的优先"是先验常识,几乎不可能过拟合;其余5个因子只算rank不参与打分。
    factor_df['prefilter_score'] = factor_df['avg_money_5_rank']
