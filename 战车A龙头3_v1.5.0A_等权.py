# =========================================================
# Strategy Version: v1.5.0A-等权 (equal-weight prefilter)
# Updated: 2026-07-06
# Status: research/backtest — UNVERIFIED 消融实验版
# Main File: 战车A龙头3_v1.5.0A_等权.py
# Source From: 战车A龙头3_v1.5.0.py
#
# === V1.5.0A 与 V1.5.0 的唯一差异(单变量消融) ===
# 250预筛 6 因子权重: 原拟合权重(0.25/0.15/0.20/0.10/0.15/0.15) → 全部等权(各 1/6)
# 目的: 检验那 6 个权重是"信息"还是"对2026拟合的装饰"。
#   同区间跑 V1.5.0A vs V1.5.0:
#   - 成绩差不多 → 权重没贡献,用等权(少6个自由参数,抗过拟合)
#   - 差很多     → 权重确实拟合了2026,样本外(2023/2025)更要警惕
# 其余(空仓闸/灾难跳空/deep_water偏向/v3打分)与 V1.5.0 完全一致,不动。
#
...
def _prefilter_dragon_auction_candidates(
    ...
    # [V1.5.0A] 消融实验: 6 因子等权(各1/6),替代原拟合权重(0.25/0.15/0.20/0.10/0.15/0.15)。
    # 等权是最经典的抗过拟合手段——损失一点样本内成绩,换样本外稳定。
    _eq = 1.0 / 6.0
    factor_df['prefilter_score'] = (
        factor_df['avg_money_5_rank'] * _eq
        + factor_df['avg_money_10_rank'] * _eq
        + factor_df['ret_5_rank'] * _eq
        + factor_df['ret_10_rank'] * _eq
        + factor_df['close_to_20d_high_rank'] * _eq
        + factor_df['range_10_rank'] * _eq
    )
